...
public class DaysInMonth {
    ...
    private static int getValidYear(Scanner scanner) {
        int year = -1;
        while (year < 0) {
            System.out.print("Enter the year (non-negative number, e.g., 1999): ");
            if (scanner.hasNextInt()) {
                ...
                System.out.println("Invalid input. Please enter a numeric year (e.g., 1999).");
                scanner.next();
            }
        }
        return year;
    ...
    private static String getValidMonth(Scanner scanner) {
        String month = "";
        while (true) {
            System.out.print("Enter the month (full name, abbreviation, 3 letters, or number, e.g., January, Jan, or 1): ");
            month = scanner.next().trim().toLowerCase();
            if (isValidMonth(month)) {
                break;
            } else {
                System.out.println("Invalid month. Please enter a valid month format (e.g., January, Jan., Jan, or 1).");
            }
        }
        return month;
    }
